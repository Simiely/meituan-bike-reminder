...
### 冷启动美团扫一扫黑屏（v2.7.1 修复）

> **现象：美团不在后台（冷启动）时，自动打开扫码页相机预览概率性黑屏，但能正常扫码。**
> **关键判断：黑屏却仍能解码 = 相机在采帧、预览 Surface 没渲染 → 是预览渲染问题，不是相机采集问题。**
> **根因：美团扫码活动在“冷进程”里 `camera.open` 早于预览 `Surface` 就绪，竞态导致黑屏；热进程（已在后台）下不出现。**
> **我们改不了美团代码，只能改触发方式：先 `startActivity` 打开美团（预热进程），延时 ~1.2s 再发扫码深链，使扫码活动在热进程中启动，复现“热=不黑”的路径。同时移除扫码 Intent 的 `FLAG_ACTIVITY_CLEAR_TOP`（热启动时它会重建美团栈，反而加剧闪屏/竞态）。**
